...
class Generator(nn.Module):
    def __init__(self, input_size, image_size, ngf, leaky_relu=False):
        ...
        for i in range(self.num_of_block-1):
            ...
            setattr(self, f'g{i}_block', nn.Sequential(g_block))
        self.last_gblock = nn.Sequential(OrderedDict([('last_conv', nn.ConvTranspose2d(in_channels=ngf // pow(2, self.num_of_block-2),
                                                                                       out_channels=3,
                                                                                       kernel_size=4,
                                                                                       stride=2,
                                                                                       padding=1))]))
...
class Discriminator(nn.Module):
    def __init__(self, image_size=128, ndf=16, leaky_relu=False):
        ...
        self.last_dblock = nn.Sequential(OrderedDict([('last_conv', nn.Conv2d(in_channels=ndf*pow(2, self.num_of_block - 1),
                                                                              out_channels=1,
                                                                              kernel_size=4,
                                                                              stride=1,
                                                                              padding=0))]))
